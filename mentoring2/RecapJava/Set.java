...
public class Set {
    ...
    In my projectö I was usıng set to make the customers unıque to prevent
    ...
    *Queue(we didn't learn)
    ...
    I do not use LinkedList a lot but efficiency of reaching the ArrayList is more then LinkedList.
    5)What is difference between Set types?
     HASHSET:   it stores the elements without any orders.--> Null values
     LINKHASHSET:it stores the elements insertion order.  --> Null values
     TREESET:   it stores the elements ascending order.   --> no null values
     LIST:
     1)List can store the dublicates elements.
